    ...
    $.extend(DatePicker.prototype, {
      fillThai: function () {
        ...
        this.picker
          ...
          .find('th:eq(2)')
        //.text(''+(year+thai.adj)) // ปิดตัวเลขที่ทำให้สับสน
      ...
      , clickThai: function (e) {
        ...
        if (target.length === 1 && target.is('.year')) {
          //  alert(this.o.startDate.getFullYear());
          if ((target.text() >= (this.o.startDate.getFullYear()) + thai.adj) && (target.text() <= (this.o.endDate.getFullYear() + thai.adj)))
            target.text(Number(target.text()) - thai.adj) // บัค ปีติดลบไปเรื่อยๆ
        }
