...
for ($i = 0; $i < count($_SESSION['food_admin']['reserve']['tables_no']); $i++) {
    $sqlreservlist = "INSERT INTO reservelist SET 
                reservlist_amount		= '" . $_SESSION['food_admin']['reserve']['seats'][$i] . "',
                reservlist_status		= '0',
                reservlist_note		    = '" . $_POST['reservlist_note_' . $i] . "',
                reserv_id			    = '" . $last_reserveid . "',
                tables_no		        = '" . $_SESSION['food_admin']['reserve']['tables_no'][$i] . "'";
...
echo '<script>alert("บันทึกการจองเรียบร้อยแล้ว\nรหัสการจองคือ ' . str_pad($last_reserveid,5,0,STR_PAD_LEFT) . '"); window.location.assign("staff_reserve_history.php")</script>';
mysqli_close($link);
